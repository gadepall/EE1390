...
\item Let $\cbrak{x_i}_{i=0}^{N-1}$ be the marks obtained by students in a class.  The objective is to award grades ranging from $A$ (highest) to $D$ (lowest) to the students.  This is done by mapping a set of marks (which are closer to each other in some sense) to  a cluster.
\item Table \ref{table:grade} shows the equivalence of each grade to a cluster.
\begin{table}[!ht]
\input{./figs/nearest.tex}
\caption{}
\label{table:grade}
\end{table}
...
\begin{lstlisting}
svn checkout https://github.com/gadepall/EE1390/trunk/manuals/nearest_neighbour/codes
python grades.py marks.xlsx
\end{lstlisting}
The \textbf{grade\_new.xlsx} file will contain the grades.
\item Modify the python script to add an $F$ grade as well.
\end{enumerate}
